...
def dispatch_webhook(merchant_id: str, event: str, payload: dict):
    ...
    body  = json.dumps({"event": event, "data": payload, "sent_at": datetime.now(timezone.utc).isoformat()})
    ...
    for hook in hooks:
        ...
        # requests.post(hook["url"], data=body, headers={"X-Payday-Signature": f"sha256={sig}"}, timeout=5)
